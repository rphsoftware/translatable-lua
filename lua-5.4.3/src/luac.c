...
{
 fprintf(stderr,LT_LUAC_ERROR_S_CANNOT_S_S_S,progname,what,output,strerror(errno));
 exit(EXIT_FAILURE);
}
...
{
 if (*message=='-')
  fprintf(stderr,LT_LUAC_ERROR_S_UNRECOPT_S,progname,message);
 else
  fprintf(stderr,"%s: %s\n",progname,message);
 fprintf(stderr,
	LT_LUAC_USAGE
...
static void PrintConstant(const Proto* f, int i)
{
 ...
 switch (ttypetag(o))
 {
  case LUA_VNIL:
	...
	break;
  case LUA_VFALSE:
	printf(LT_TOKEN_FALSE);
	break;
  case LUA_VTRUE:
	printf(LT_TOKEN_TRUE);
	break;
  case LUA_VNUMFLT:
	{
	char buff[100];
